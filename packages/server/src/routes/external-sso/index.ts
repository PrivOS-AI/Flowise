...
router.get('/', async (req: Request, res: Response) => {
    try {
        ...
        if (roles.includes('admin')) {
            ...
            permissions = ['chatflows:view', 'chatflows:create', 'credentials:view', 'credentials:create']
            logger.info('[ExternalSSO]: User has user role - granted view and create permissions')
        } else {
