...
export class AddRoomIdToEntities1730000000000 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`chat_flow\` ADD \`roomId\` TEXT NULL`)

        // Add roomId column to credential table
        await queryRunner.query(`ALTER TABLE \`credential\` ADD \`roomId\` TEXT NULL`)

        // Add roomId column to tool table
        await queryRunner.query(`ALTER TABLE \`tool\` ADD \`roomId\` TEXT NULL`)

        // Add roomId column to assistant table
        await queryRunner.query(`ALTER TABLE \`assistant\` ADD \`roomId\` TEXT NULL`)

        // Add roomId column to execution table
        await queryRunner.query(`ALTER TABLE \`execution\` ADD \`roomId\` TEXT NULL`)

        // Add roomId column to variable table
        await queryRunner.query(`ALTER TABLE \`variable\` ADD \`roomId\` TEXT NULL`)

        // Add roomId column to document_store table
        await queryRunner.query(`ALTER TABLE \`document_store\` ADD \`roomId\` TEXT NULL`)
    }
