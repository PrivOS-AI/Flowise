...
async function generateVideo(args) {
    ...
    const personGeneration = args.person_generation || 'dont_allow'
 ...
 */
async function generateVideoFromImage(args) {
    const imageBase64 = args.image_base64
    const imageMimeType = args.image_mime_type || 'image/jpeg'
    const prompt = args.prompt || 'Animate this image naturally'
    const aspectRatio = args.aspect_ratio || '16:9'
    const durationSeconds = parseInt(args.duration_seconds) || 5
    const negativePrompt = args.negative_prompt
    const modelName = getDefaultModel()
